...
from symmetrizer.nn import BasisLunarNetworkWrapper, BasisLunarLayer, \
    BasisLinear


class LunarFfModel(torch.nn.Module):
...
class LunarBasisModel(torch.nn.Module):
    ...
    def __init__(
        ...
        super(LunarBasisModel, self).__init__()
        input_size = image_shape[0]
        input_size = 1


        self.head = BasisLunarNetworkWrapper(input_size, fc_sizes,
                                             gain_type=gain_type,
                                             basis=basis)
        self.pi = BasisLunarLayer(fc_sizes[-1], 1,
                                         gain_type=gain_type,
                                         basis=basis)
        self.value = BasisLunarLayer(fc_sizes[-1], 1,
                                            gain_type=gain_type,
